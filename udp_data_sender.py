"""
UDP Data Sender - 通用UDP数据发送工具

一个可视化的UDP数据发送工具，可以将Excel文件中的数据通过UDP协议发送到指定的网络地址。
支持完全图形化配置，自动生成MATLAB解析脚本。
...
class MessageSenderGUI:
    ...
    def __init__(self, root):
        """初始化GUI界面"""
        self.root = root
        self.root.title("UDP Data Sender - 通用数据发送工具")
    ...
    def setup_ui(self):
        ...
        ttk.Button(file_frame, text="生成示例Excel", command=self.generate_example_excel).grid(row=0, column=3, padx=(10,0))
    ...
    def generate_example_excel(self):
        """生成包含通用表头和示例数据的Excel模板"""
        try:
            # 通用列名：第一列是时间戳，其余列包含整数与浮点示例
            columns = [
                'Timestamp',
                'Speed_Ref_Int',
                'Gross_Weight_Int',
            ...
            # 构造示例数据（5 行）
            data = []
            base_seconds = 1 * 3600 + 2 * 60 + 3  # 01:02:03
            for i in range(5):
                seconds = base_seconds + i
                timestamp = f"01:02:{3+i:02d}:{123456 + i*100}"  # 简单示例
                row = [
                    timestamp,
                    120 + i,          # Speed_Ref_Int
                    50000 + i * 10,   # Gross_Weight_Int
                    i % 2,            # Gear_Status_Int
                    10000.0 + i * 50, # Altitude_Double
                    250.5 + i * 1.2   # Airspeed_Double
                ]
                data.append(row)
            ...
            filename = filedialog.asksaveasfilename(
                ...
                initialfile="udp_example.xlsx"
            )
            if not filename:
                return
            
            with pd.ExcelWriter(filename, engine='openpyxl') as writer:
                df.to_excel(writer, sheet_name=self.sheet_name.get() or 'A', index=False)
            
            # 将整数列示例同步到界面
            self.int_columns_widget.delete("1.0", tk.END)
            self.int_columns_widget.insert("1.0", "Speed_Ref_Int\nGross_Weight_Int\nGear_Status_Int")
            self.file_path.set(filename)
            
            messagebox.showinfo("成功", f"已生成示例Excel: {filename}")
            self.log_message(f"已生成示例Excel: {filename}")
        except Exception as e:
            messagebox.showerror("错误", f"生成示例Excel失败: {e}")
